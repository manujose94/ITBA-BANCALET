...
@Repository
public class AyudaHibernateDao extends HibernateSQL implements AyudaDao {
	...
	public Collection<Ayuda> findByAsunto(String asunto) {
		asunto = formatSQL(asunto);
		final Query query = em.createNativeQuery("select * from ayuda as u where lower(u.asunto) like :asunto ESCAPE '/'",
				Ayuda.class);
	...
	public Collection<Ayuda> findByName(String name) {
		name = formatSQL(name);
		final Query query = em.createNativeQuery("select * from ayuda as u where lower(u.name) like :name ESCAPE '/'", Ayuda.class);
	...
	public Collection<Ayuda> findByInforme(String informe) {
		informe = formatSQL(informe);
		final Query query = em.createNativeQuery("select * from ayuda as u where lower(u.informe) like :informe ESCAPE '/'",
				Ayuda.class);
	...
	public Collection<Ayuda> findByMensaje(String mensaje) {
		mensaje = formatSQL(mensaje);
		final Query query = em.createNativeQuery("select * from ayuda as u where lower(u.mensaje) like :mensaje ESCAPE '/'",
				Ayuda.class);
	...
	public Collection<Ayuda> findByOutsideEstado(int estado, String asunto) {
		...
		final Query query = em.createNativeQuery(
				"select * from ayuda as u where u.estado = :estado and u.userId = :userId and lower(u.asunto) like :asunto ESCAPE '/'",
				Ayuda.class);
		query.setParameter("estado", estado);
		query.setParameter("userId", Ayuda.OUT);
		query.setParameter("asunto", asunto);
