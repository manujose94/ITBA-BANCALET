...
public interface UserService {
	...
	/**
	 * Encuentra el usuario de la Sesion actual.
	 *
	 * @param principal Objeto que debe ser UserDetails .
	 * @return El usuario cuyo username corresponda con el Object dado.
	 */
	User findUserSessionByUsername(Object principal);

	/**
	 * Codifica una imagen en base64 a partir de bytes.
	 *
	 * @param imagen Bytes de la imagen a codificar.
	 * @return La imagen decodificada en base64.
	 */
	String getImageUsertoBase64encoded(byte[] imagen);
	...
	/**
	 * Devuelve todos los usuarios que tienen el username similar al dado.
	 * 
	...
	User updateUser(User user, String telf, String city, String country, String code, String direccion, int numImg,
			String urlImg, double lat, double lon, CommonsMultipartFile[] fileUpload);

	User updateUserPass(User userIndex, String password);

	Collection<Item> getSugerencias(long userId);

	long getUserTotalSales(User user);

	boolean getMaxVendor(User user);

	boolean getMaxBuyer(User user);

	boolean getMaxRated(User user);

	Collection<User> findByidCompradorconHistorial(long idComprador);

	Validation confirmEmail(User user);
