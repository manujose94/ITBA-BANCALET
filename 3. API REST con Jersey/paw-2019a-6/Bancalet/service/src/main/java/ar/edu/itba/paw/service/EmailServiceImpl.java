...
public class EmailServiceImpl implements EmailService {
	...
	private static final int TIPE_MESSAGE_REGISTER = 4;
	...
	public Validation registerUserMail(User userIndex) {
		String confirmUrl = env.getProperty("app.url") + "confrimregister/" + userIndex.getUserId();
		...
		String mensajeUser = messageSource.getMessage("user.register_message_mail", parametersM,
				LocaleContextHolder.getLocale());
		String mensajeMail = createHtmlEmail(mensajeUser, userIndex.getUsername(), userIndex.getEmail(), confirmUrl,
				TIPE_MESSAGE_REGISTER);
	...
	public Validation bajaUserMail(User userIndex) {
		...
		Date fecha_baja = new Date(calendar.getTime().getTime());
	...
	public Validation altaUserMail(User userIndex) {
		Calendar calendar = Calendar.getInstance();
		Date fecha_alta = new Date(calendar.getTime().getTime());
	...
	public Validation changePassMail(boolean equals, User userIndex, String clarepassword) {
		...
		Calendar calendar = Calendar.getInstance();
		Date fecha_cambio = new Date(calendar.getTime().getTime());
	...
	private String createHtmlEmail(String text, String name, String email, Object auxiliar, int tipo) {
		...
		case TIPE_MESSAGE_ITEM:
			...
			html = "<html><head><style> table {font-family: arial, sans-serif; border-collapse: collapse; width: 100%; }"
					...
					+ "</a></p></body></html>";
			break;
		case TIPE_MESSAGE_REGISTER:
			String confirmUrl = (String) auxiliar;
			html = "<html>" + "  <body>" + "  	<h4>" + text + "<a href=" + confirmUrl + ">" + confirmUrl + "</a>"
					+ "</h4>" + "    <p>" + donotreply + "</p>" + "    <p>" + "    	<a href=" + contactUrl + ">"
					+ contactar + "</a>" + "    </p>" + "  </body>" + "</html>";
			break;
		}
		return html;
	...
	@Override
	public Validation confirmUserMail(User userIndex) {
		Object[] parametersM = { userIndex.getUsername() };
		String asunto = messageSource.getMessage("user.register_subject_mail_confirm", null,
				LocaleContextHolder.getLocale());
		String mensajeUser = messageSource.getMessage("user.register_message_mail_confirm", parametersM,
				LocaleContextHolder.getLocale());
		String mensajeMail = createHtmlEmail(mensajeUser, userIndex.getUsername(), userIndex.getEmail(), null,
				TIPE_MESSAGE_TEXT);
