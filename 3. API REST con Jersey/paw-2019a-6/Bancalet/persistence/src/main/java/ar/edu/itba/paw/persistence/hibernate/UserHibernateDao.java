...
@Repository
public class UserHibernateDao extends HibernateSQL implements UserDao {
	...
	public Collection<User> filterByName(String username) {
		username = formatSQL(username);
		final Query query = em.createNativeQuery("select * from users as u where lower(u.username) like :username ESCAPE '/'",
				User.class);
		query.setParameter("username", username);
		Collection<User> list = null;
		try {
	...
	public Collection<User> findByFilter(String username, String role) {
		username = formatSQL(username);
		final Query query = em.createNativeQuery("select * from users as u where u.role = :role and lower(u.username) like :username ESCAPE '/'", User.class);
		query.setParameter("username",username);
		query.setParameter("role", role);
		Collection<User> list = null;
		try {
	...
	public Collection<User> findByNameEstado(String name, int estado) {
		...
		final Query query = em
				.createNativeQuery("select * from users as u where u.estado = :estado and lower(u.username) like :name ESCAPE '/'", User.class);
	...
	public Collection<User> findByFilterEstado(String name, String role, int estado) {
		...
		final Query query = em.createNativeQuery(
				"select * from users as u where u.role = :role and u.estado = :estado and lower(u.username) like :name ESCAPE '/'",
				User.class);
		query.setParameter("name", name);
		query.setParameter("estado", estado);
		query.setParameter("role", role);
