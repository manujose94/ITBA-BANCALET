...
public class ItemHibernateDao implements ItemDao {
	...
	public Collection<Item> findByItemname(String name) {
		...
		final TypedQuery<Item> query = em.createQuery("from Item as u where lower(u.name) like :name", Item.class);
		...
		if (tipo == 0) {
			if (antesDespues == 0) {
				query = em.createQuery(
						"from Item as u where u.estado = :estado and lower(u.name) like :name and u.price between :min and :max",
						Item.class);
				...
				query = em.createQuery(
						"from Item as u where u.estado = :estado and lower(u.name) like :name and u.price between :min and :max and u.fechaCaducidad <= :date",
						Item.class);
				query.setParameter("estado", estado);
				query.setParameter("name", "%" + name + "%");
				query.setParameter("max", max);
			...
			if (antesDespues == 0) {
				query = em.createQuery(
						"from Item as u where u.tipo = :tipo and u.estado = :estado and lower(u.name) like :name and u.price between :min and :max",
						Item.class);
				...
				query = em.createQuery(
						"from Item as u where u.tipo = :tipo and u.estado = :estado and lower(u.name) like :name and u.price between :min and :max and u.fechaCaducidad <= :date",
						Item.class);
				query.setParameter("tipo", tipo);
				query.setParameter("estado", estado);
				query.setParameter("name", "%" + name + "%");
