...
public class TokenHandler {
	...
	public String createTokenForUser(String username, Collection<? extends GrantedAuthority> authorities) {
		...
		return Jwts.builder().setId(UUID.randomUUID().toString()).setSubject(username)
				...
				.setExpiration(expiration).setIssuedAt(Date.from(now.toInstant()))
				.signWith(SignatureAlgorithm.HS512, tokenSigningKey).compact();
	}

	public UserDetails parseUserDetailsFromToken(String token) {
		try {
			final String username = Jwts.parser().setSigningKey(tokenSigningKey).parseClaimsJws(token).getBody()
					.getSubject();
